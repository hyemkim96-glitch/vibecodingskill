...
> 이 파일을 코딩 에이전트(Claude, GPT, Cursor 등)에 붙여넣을 때, 아래 5가지를 반드시 지켜주세요.

1. **아래 값은 근사치가 아닌 정확한 값입니다.** hex/px를 임의로 반올림하거나 비슷한 값을 새로 만들지 마세요.
2. **hex 대신 CSS 변수명을 참조하세요.** 코드를 생성할 때 \`#3D6DFF\` 같은 원시값이 아니라 \`var(--color-fill-normal)\` 같은 토큰 변수명을 쓰세요.
3. **4계층 순서(Palette → Semantic → Role → Variant)를 지키세요.** 계층을 건너뛰어 참조하지 마세요 — 예를 들어 컴포넌트에서 Palette를 직접 참조하지 말고 반드시 Role 토큰을 거치세요.
4. **이 파일은 핵심 토큰의 서브셋만 담고 있습니다.** 전체 토큰 목록이 필요하면 이 브랜드의 CSS 변수 export를 함께 붙여넣어 주세요 — Design.md 하나만으로는 일부 토큰이 누락될 수 있습니다.
5. **긴 세션에서는 이 지시문과 아래 Do/Don't를 주기적으로 다시 확인하세요.** 화면을 여러 개 연달아 만들다 보면 토큰 대신 임의의 값을 쓰게 되는 경우가 흔합니다.
