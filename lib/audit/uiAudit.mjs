...
export function serviceInspector(files) {
  ...
  for (const f of files) {
    ...
    f.code.split('\n').forEach((line, i) => {
      ...
      // S-CONSISTENT-TABS: 같은 레벨 필터/카테고리 탭은 공유 컴포넌트(PillTabs)를
      // 써야 한다. 인라인 pill 탭(rounded-full + onClick 버튼)을 직접 만들면 위반.
      if (/rounded-full/.test(line) && /onClick/.test(line) && !/PillTabs/.test(f.code))
        add('S-CONSISTENT-TABS', 'warn', f.path, i + 1,
          '인라인 필터 탭 구현 → 공유 컴포넌트 <PillTabs> 사용 (같은 레벨 UI는 동일 컴포넌트)');
...
export function wireframeInspector(files) {
  ...
  for (const f of files) {
    ...
    f.code.split('\n').forEach((line, i) => {
      ...
      // W-SPACING: 레이아웃 여백(gap/margin/padding)은 파운데이션 토큰(space.*)을 따라야 함.
      // 0과 토큰 연산(space.* / 템플릿 리터럴)은 허용, bare 숫자 리터럴만 위반.
      const sp = line.match(/\b(gap|rowGap|columnGap|margin(?:Top|Bottom|Left|Right)?|padding(?:Top|Bottom|Left|Right)?)\s*:\s*(\d+)\b/);
      if (sp && sp[2] !== '0')
        add('W-SPACING', 'warn', f.path, i + 1, `하드코딩 여백 ${sp[1]}: ${sp[2]} → 파운데이션 토큰(space.*) 사용`);
