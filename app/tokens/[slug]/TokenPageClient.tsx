...
/** 카테고리/플랫폼별 대표 UI 패턴 — 토큰 실제 값 반영 + 인터랙션 */
function BrandUIPreview({ token, platform }: { token: BrandToken; platform: 'mobile' | 'web' }) {
  ...
  const Btn = ({ children, variant = 'primary', disabled = false, style = {} }: {
    children: React.ReactNode; variant?: 'primary' | 'secondary' | 'ghost'; disabled?: boolean; style?: React.CSSProperties;
  }) => (
    <div
      data-ui="btn"
      ...
      style={{
        ...
        borderRadius: btnRadius, fontSize: '12px', fontWeight: 600, cursor: 'pointer',
        background: variant === 'primary' ? primary : variant === 'secondary' ? cardBg : 'transparent',
        color: variant === 'primary' ? onPrimary : textMain,
        border: variant === 'secondary' ? `1px solid ${border}` : 'none',
  ...
  const Card = ({ children, style = {} }: { children: React.ReactNode; style?: React.CSSProperties }) => (
    <div data-ui="card" style={{ background: cardBg, borderRadius: cardRadius, overflow: 'hidden', ...style }}>
      {children}
    </div>
  );

  const Row = ({ children, style = {} }: { children: React.ReactNode; style?: React.CSSProperties }) => (
    <div data-ui="row" style={{ display: 'flex', alignItems: 'center', ...style }}>{children}</div>
  );

  const Chip = ({ children, active = false }: { children: React.ReactNode; active?: boolean }) => (
    <div data-ui="chip" style={{
      padding: '5px 12px', borderRadius: chipRadius, fontSize: '12px', fontWeight: 500, whiteSpace: 'nowrap',
      background: active ? primary : cardBg, color: active ? onPrimary : textSub,
    ...
    if (isMobile) {
      return (
        <div className={styles.uiPreview} style={{ background: bg, gap: `${gap}px` }}>
          <Card style={{ padding: `${pad}px`, display: 'flex', flexDirection: 'column', gap: '6px' }}>
          ...
          <div style={{ display: 'flex', gap: `${gap}px` }}>
            {(['이체', '충전', '조회'] as const).map((label, i) => (
              <Btn key={label} variant={i === 0 ? 'primary' : 'secondary'} style={{ flex: 1, padding: '10px 0' }}>{label}</Btn>
            ))}
          </div>
          {[
            { name: '스타벅스', sub: '카드결제', amount: '-6,500원', color: textMain },
            { name: '급여', sub: '입금', amount: '+3,200,000원', color: accent },
          ].map((item, i) => (
            <Row key={item.name} style={{
              justifyContent: 'space-between', padding: `${gap}px 0`,
          ...
          <Btn variant="primary" disabled style={{ width: '100%', padding: '11px' }}>이체 불가 (잔액 부족)</Btn>
        </div>
      );
    }
    return (
      <div className={styles.uiPreview} style={{ background: bg, flexDirection: 'row', alignItems: 'stretch', gap: `${gap * 2}px`, padding: `${pad}px` }}>
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: `${gap}px` }}>
          ...
          <div data-ui="card" style={{ background: primary, borderRadius: cardRadius, padding: `${pad}px` }}>
        ...
        <div style={{ flex: 2, display: 'flex', flexDirection: 'column', gap: `${gap}px` }}>
          {[
            { name: '스타벅스', cat: '카페', amount: '-6,500', color: textMain },
            { name: '급여', cat: '입금', amount: '+3,200,000', color: accent },
            { name: '넷플릭스', cat: '구독', amount: '-17,000', color: textMain },
          ].map((item) => (
            <Row key={item.name} data-ui="row" style={{
              justifyContent: 'space-between',
              padding: `${gap}px ${Math.round(pad * 0.75)}px`, background: cardBg, borderRadius: inputRadius,
            }}>
              <div>
                <span style={{ fontSize: '13px', color: textMain, fontWeight: 500 }}>{item.name}</span>
                <span style={{ fontSize: '11px', color: textSub, marginLeft: '8px' }}>{item.cat}</span>
              </div>
              <span style={{ fontSize: '13px', fontWeight: 600, color: item.color }}>{item.amount}원</span>
            </Row>
          ))}
          <Btn variant="primary" style={{ width: '100%', padding: '10px' }}>이체하기</Btn>
        </div>
      </div>
    );
  }
  ...
  if (category === '플랫폼') {
    if (isMobile) {
      return (
        <div className={styles.uiPreview} style={{ background: bg, gap: `${gap}px` }}>
          <div data-ui="input" style={{ background: cardBg, borderRadius: inputRadius, padding: '8px 12px', fontSize: '12px', color: textMuted, border: `1px solid ${border}` }}>
            음식, 가게 검색
          </div>
          ...
          <Card>
            ...
            <div style={{ padding: '12px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
              ...
              <Btn variant="primary" style={{ alignSelf: 'flex-start', padding: '6px 14px', marginTop: '4px', fontSize: '11px' }}>주문하기</Btn>
            </div>
          </Card>
        </div>
      );
      ...
      <div className={styles.uiPreview} style={{ background: bg, gap: `${gap}px`, padding: `${pad}px` }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: `${gap}px` }}>
          {['맛있는 치킨', '행복한 피자', '우리동네 한식'].map((name, i) => (
            <Card key={name} style={{ border: `1px solid ${border}` }}>
              <div style={{ height: '70px', background: `linear-gradient(135deg, ${primary}22, ${accent}22)` }} />
  ...
  if (category === '커머스') {
    if (isMobile) {
      return (
        <div className={styles.uiPreview} style={{ background: bg, gap: `${gap}px` }}>
          <div data-ui="card" style={{ background: primary, borderRadius: cardRadius, padding: `${gap * 2}px ${pad}px`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
            ...
            <Btn variant="ghost" style={{
              background: onPrimary === '#ffffff' ? 'rgba(255,255,255,0.2)' : 'rgba(0,0,0,0.1)',
              color: onPrimary, padding: '5px 10px', fontSize: '11px',
            }}>보러가기</Btn>
          </div>
            ...
            {[{ name: '무선 이어폰', price: '89,000', discount: '35%' }, { name: '스마트워치', price: '249,000', discount: '22%' }].map((item) => (
              <Card key={item.name}>
                <div style={{ height: '80px', background: border, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <span style={{ background: accent, borderRadius: badgeRadius, padding: '2px 6px', fontSize: '10px', fontWeight: 700, color: getContrastColor(accent) }}>{item.discount}</span>
                </div>
          ...
          <Btn variant="primary" style={{ width: '100%', padding: '11px', fontSize: '13px', fontWeight: 700 }}>장바구니 담기</Btn>
          <Btn variant="secondary" disabled style={{ width: '100%', padding: '11px', fontSize: '13px' }}>품절</Btn>
        </div>
      );
    }
    return (
      <div className={styles.uiPreview} style={{ background: bg, padding: `${pad}px`, gap: `${gap}px` }}>
        <Row style={{ justifyContent: 'space-between' }}>
          ...
          <span data-ui="btn" style={{ fontSize: '11px', color: primary, fontWeight: 600, cursor: 'pointer' }}>전체보기 →</span>
        </Row>
          ...
          {['무선이어폰', '스마트워치', '노트북', '태블릿'].map((name, i) => (
            <Card key={name} style={{ border: `1px solid ${border}` }}>
              <div style={{ height: '60px', background: border, position: 'relative' }}>
                <span style={{ position: 'absolute', top: '4px', left: '4px', background: primary, color: onPrimary, borderRadius: badgeRadius, padding: '2px 5px', fontSize: '9px', fontWeight: 700 }}>
                  {[35, 22, 18, 40][i]}%
                </span>
              </div>
              <div style={{ padding: '8px' }}>
      ...
      return (
        <div className={styles.uiPreview} style={{ background: bg, gap: `${gap}px` }}>
          <Row style={{ gap: `${gap}px` }}>
            <div data-ui="input" style={{ flex: 1, background: cardBg, borderRadius: inputRadius, padding: '8px 12px', fontSize: '12px', color: textMuted, border: `1px solid ${border}` }}>우리 동네 검색</div>
            <Btn variant="primary" style={{ padding: '8px 12px' }}>글쓰기</Btn>
          </Row>
          {[
            { title: '아이폰 15 팔아요', loc: '서초구 · 10분 전', price: '900,000원' },
            { title: '자전거 삽니다', loc: '강남구 · 1시간 전', price: '50,000원' },
          ].map((item, i) => (
            <Row key={item.title} style={{
              gap: '12px', padding: `${gap}px 0`, alignItems: 'flex-start',
              borderBottom: i === 0 ? `1px solid ${border}` : 'none',
            }}>
              <div data-ui="card" style={{ width: '60px', height: '60px', background: cardBg, borderRadius: cardRadius, flexShrink: 0 }} />
  ...
  if (isMobile) {
    return (
      <div className={styles.uiPreview} style={{ background: bg, gap: `${gap}px` }}>
        <Card style={{ padding: `${pad}px`, border: `1px solid ${border}`, display: 'flex', flexDirection: 'column', gap: '6px',
          ...
          <Btn variant="primary" style={{ alignSelf: 'flex-start', padding: '7px 16px', marginTop: '4px' }}>지금 보기</Btn>
        </Card>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: `${gap}px` }}>
        ...
        <Btn variant="secondary" disabled style={{ width: '100%', padding: '10px', fontSize: '12px' }}>사이즈 선택 후 구매 가능</Btn>
      </div>
    );
  }
  return (
    <div className={styles.uiPreview} style={{ background: bg, padding: `${pad}px`, gap: `${gap}px` }}>
      <div style={{ display: 'flex', gap: `${gap * 2}px` }}>
        <Card style={{ flex: 2, padding: `${pad}px`, border: `1px solid ${border}`, display: 'flex', flexDirection: 'column', gap: '6px' }}>
          <div style={{ fontSize: '10px', letterSpacing: '0.08em', color: textSub, textTransform: 'uppercase' }}>Featured</div>
          <div style={{ fontSize: '20px', fontWeight: 700, color: textMain, letterSpacing: '-0.01em', lineHeight: 1.2 }}>{token.tagline}</div>
          <Btn variant="primary" style={{ alignSelf: 'flex-start', padding: '8px 20px', marginTop: '6px' }}>쇼핑하기</Btn>
        </Card>
